...
const SensitivityMonitor = ({ S, setS, version, activeTab }) => {
  ...
  const SENSITIVITY_CAPACITY = 75;
  useEffect(() => {
    const enabledCount = Object.values(S || {}).filter(s => s?.status === 'active').length;
    setUsagePercentage(Math.round((enabledCount / SENSITIVITY_CAPACITY) * 100));
  }, [S]);
                ...
                <div className="header-content">
                  ...
                  <div className="usage-indicator">
                    ...
                    <div className="usage-details">
                      {Object.values(S || {}).filter(s => s?.status === 'active').length} of {SENSITIVITY_CAPACITY} variables enabled
                    </div>
                  </div>
                </div>
                <button
