...
import './styles/HomePage.CSS/ComparisonWorkspace.css';
// Phase 4: Mutation ledger styles
import './styles/HomePage.CSS/MutationLedger.css';
// Phase 5: Scorecard styles
import './styles/HomePage.CSS/ComparisonScorecard.css';
...
const HomePageContent = () => {
    ...
    const handleRun = async () => {
        ...
        try {
            // Phase 3: Use explicit baseline assembly instead of ad-hoc useSummaryItems patching.
            // buildRunPayload() assembles directInputs + admittedDerivedFacts deterministically
            // and produces the same backend-compatible payload shape.
            const { payload: requestPayload, assembly } = buildRunPayload(
            ...
            console.log(
                '[Phase 3] CFA run — explicit baseline assembly:',
                `v${version}`,
                `admitted: ${assembly.admittedDerivedFacts.length}`,
                `excluded: ${assembly.excludedDerivedFacts.length}`,
            );
    ...
    const handleRuns = async () => {
        ...
        try {
            // Phase 4: Use explicit baseline assembly (same as handleRun).
            // The sensitivity-specific orchestration steps below are PRESERVED as
            // orchestration/reporting tools — they are not state-authoring layers.
            // SenParameters (S) remains the sensitivity configuration; the formValues
            // and summaryItems portions are now built deterministically.
            const { payload: requestPayload, assembly } = buildRunPayload(
                version,
                {
                    selectedVersions,
            ...
            console.log(
                '[Phase 4] Sensitivity run — explicit baseline assembly:',
                `v${version}`,
                `admitted: ${assembly.admittedDerivedFacts.length}`,
                `excluded: ${assembly.excludedDerivedFacts.length}`,
                `SenParameters active: ${Object.values(S).filter(e => e.enabled).length}`,
            );
            console.log('Running sensitivity with parameters:', requestPayload);
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            ...
            case 'DurationMatrix':
                return <DurationConfigMatrix formValues={formValues} baseYear={2026} />;

            case 'Compare':
                return (
                    <ComparisonWorkspace
                        formMatrix={formValues}
                        plantLifetime={formValues?.plantLifetimeAmount10
                            ? (formValues.plantLifetimeAmount10?.matrix?.v1?.z1
                        ...
                        getMutationLedger={getMutationLedger}
                    />
                );
    ...
    />
            ...
            <div className="main-content">
                ...
                <div className={`content-container${activeTab === 'DurationMatrix' ? ' dcm-tab-active' : ''}`}>
                    {activeTab !== 'AboutUs' &&
                    ...
                    <div className={`HomePageTabContent${activeTab === 'DurationMatrix' ? ' dcm-tab-active' : ''}`}>
                        {renderTabContent()}
                    </div>
                </div>
            </div>
